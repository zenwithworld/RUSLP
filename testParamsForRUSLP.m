...
%bad performace
% alpha = [10^-3, 10^-2,10^-1,1,10^1,10^2,10^3];
% beta = [10^-3, 10^-2,10^-1,1,10^1,10^2,10^3];
% gamma = [10^-3, 10^-2,10^-1,1,10^1,10^2,10^3];
% lambda = [10^-3, 10^-2,10^-1,1,10^1,10^2,10^3];
% gamma = [1];
T = 50;
S = 50;
c = nClass;
...
t_start = clock;
best_newfea = zeros(length(FeaNumCandi),1024);;
best_im = zeros(length(FeaNumCandi),1024);;
...
%             disp(['alpha=',num2str(alpha),',','beta=',num2str(beta),',','nu=',num2str(nu)]);
            disp(['al=', num2str(al), ',', 'be=', num2str(be), ',', 'gam=', num2str(100), ',', 'lam=', num2str(lam)]);
%              disp(['al=', num2str(al), ',', 'be=', num2str(be), ',', 'gam=', num2str(gam)]);
            result_path = strcat(dataset,'\','al=', num2str(al),',', 'be=', num2str(be), ',', 'gam=', num2str(100), ',', 'lam=', num2str(lam),'_result.mat');
%             result_path = strcat(dataset,'\','al=', num2str(al),',', 'be=', num2str(be), ',', 'gam=', num2str(gam),'_result.mat');
            mtrResult = [];
%             best_fea = [];
%             Z = zeros(size(fea,1),length(unique(gnd)));
%             W = RSFS(fea,L,Z,Y,alpha,beta,nu,maxIter);
%             X_2 = RUSLP(fea, nClass, W, F, G, L, al, be, gam, T, S);
            
            [dumb, X_2] = RUSLP(fea, nClass, W, F, G, L, al, be, 100, lam, T, S);
            X_2 = X_2';

%             [dumb idx] = sort(sum(Weight.*Weight,2),'descend');
            orderFeature_path = strcat(dataset,'\','feaIdx_','al=', num2str(al), ',', 'be=', num2str(be), ',', 'gam=', num2str(100), ',', 'lam=', num2str(lam),'.mat');
%             orderFeature_path = strcat(dataset,'\','feaIdx_','al=', num2str(al), ',', 'be=', num2str(be), ',', 'gam=', num2str(gam),'.mat');
            ...
            for feaIdx = 1:length(FeaNumCandi)
                ...
                if mNMI_max > bestNMI_max(feaIdx)
                    ...
                    best_newfea(feaIdx, :) = X_2';
                    best_im(feaIdx, :) = dumb;
                                        
                end
                mtrResult = [mtrResult,[feaNum,mNMI_max,sNMI_max,mNMI_sqrt,sNMI_sqrt,mACC,sACC, al, be, 100, lam]'];
%                 mtrResult = [mtrResult,[feaNum,mNMI_max,sNMI_max,mNMI_sqrt,sNMI_sqrt,mACC,sACC, al, be, gam]'];
...
save(result_path,'best_newfea', 'best_im');
t_end = clock;
disp(['exe time: ',num2str(etime(t_end,t_start))]);

%save the best results among all the parameters
result_path = strcat(dataset,'\','best','_result_',dataset,'_DCUFS','.mat');
